...
E_LINE = ''.join([' ' for i in range(34)])  # very dirty way to print an empty line in a intro screen
...
class Game:
    ...
    def updateState(self):
        ...
        if (self.future_state == Game.ST_INTRO):
            Game.pl.setEnviron(IntroScreen(
                "                                  In this Ludum Dare#29 game,you are the brave hero who must save the princess!! Unfortunately, due  to drunk python coders the game is   super bugged.  While  you    walk towards her, you bump into a BIG  bug and fall into the depths of    the  software,beneath the surface ..." + \
    ...
    def play_fall(self, time):
        """
        joue "cinématique" ou l'avatar tombe dans un puits de données
        """
    ...
    @staticmethod
    def its_time_to_create_bonus():
        # avant:
        # return random.randint(0, 4096) == 42:
        # maintenant:
        return random.randint(1, 200) == 1  # 0.5% de chance

    def play_platformer(self, time):
        """
        lance le jeu en lui meme (platformer)
        """
        if Game.pl.hasWon():
        ...
        if Game.pl.environ.current_bonus:
            Game.pl.environ.current_bonus.collide(Game.pl)

        # gestion bonus
        if (Game.pl.environ.current_bonus is None) or (not Game.pl.environ.current_bonus.visible):

            if Game.its_time_to_create_bonus():  # proba faible
                x_bonus, y_bonus = Game.pl.environ.generateBonus()
    ...
    def refresh_screen(self):
        ...
        if cond1 or cond2:
            ...
            # TODO cest le joueur qui pourrait declencher le dessin du terrain
